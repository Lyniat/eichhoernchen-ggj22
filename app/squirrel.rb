class Squirrel
  ...
  def hand_move
    ...
    unless @moving_left_hand
      @rot_left += (@input.keyboard.key_held.a ? -1 : (@input.keyboard.key_held.d ? 1 : 0)) * speed
      if @rot_left < 0
        @rot_left += 360
      end
      if @rot_left > 360
    ...
    unless @moving_right_hand
      @rot_right ||= 0
      @rot_right += (@input.keyboard.key_held.left ? -1 : (@input.keyboard.key_held.right ? 1 : 0)) * speed
      if @rot_right < 0
        @rot_right += 360
      end
      if @rot_right > 360
